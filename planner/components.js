    ...
    <${Modal} title=${editing ? "Проект" : "Новый проект"} onClose=${onClose}
      ...
      <form onSubmit=${submit} class="form">
        ...
        <div class="field"><label>Цвет</label>
          <div class="swatches">
            ${COLORS.map(c => html`<button type="button" key=${c} title="Цвет"
              class=${"swatch" + (color === c ? " sel" : "")} style=${`background:${c};`}
              onClick=${() => setColor(c)}>${color === c ? Icon.check() : ""}</button>`)}
            <label class=${"swatch swatch-custom" + (!COLORS.includes(color) ? " sel" : "")} title="Свой цвет"
              style=${!COLORS.includes(color) ? `background:${color};` : ""}>
              ${!COLORS.includes(color) ? Icon.check() : ""}
