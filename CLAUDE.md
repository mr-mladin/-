...
В репозитории три независимых приложения, каждое в своей папке. В корне —
лончер (ссылки на /finance/ и /planner/) и «выключатель» старого корневого service worker.
...
marketing/              — приложение «Маркетинг»: отчётность по рекламе (адрес .../marketing/), самостоятельное
  index.html            — точка входа, importmap, тема
  src/
    main.js             — render <App/>
    App.js              — экран дашборда + компоненты (Card, Sparkline, KPI)
    data.js             — данные проектов (сейчас демо; форма строки = будущая строка БД)
    format.js           — форматирование чисел (₽, %, разряды)
    styles.css          — стили только этого приложения
```
...
Маркетинг не зависит от кода финансов и планера. Сейчас данные демонстрационные
(в `marketing/src/data.js`), база не используется — задеть чужие данные невозможно.
Когда подключим: отдельная схема БД `marketing`, доступ клиентам — приватная ссылка
без логина (каждый видит только свой проект). Подробности и бэклог — `marketing/README.md`.

**Правило раздельности:** одно приложение = одна папка верхнего уровня + свой адрес
`.../<app>/` + своя схема БД. Приложения не делят код и стили; общее — только проект
Supabase и вход. Имя папки всегда говорит, к какому приложению относится файл.
...
# маркетинг: http://localhost:8000/marketing/
```
